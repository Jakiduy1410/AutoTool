...
def get_pid(package: str):
    out = sh(f"pidof {package}")
    if out and not out.startswith("__ERR__"):
        try:
            return int(out.split()[0])
        except Exception:
            pass
    out = sh(f"ps -A | grep -F ' {package}' || true")
    if not out:
        return None
...
def join_game(package: str, join_url: str, fallback_url: str):
    """
    Ưu tiên đúng cú pháp bạn test:
      am start -a VIEW -d "<url>" <package>
    Fallback:
      am start -a VIEW -d "<url>" -p <package>
    """
    if join_url:
        # style 1: url + package (no -p)
        sh(f"am start -a android.intent.action.VIEW -d '{join_url}' {package}", root=True)
        time.sleep(1)
        # style 2: -p package (fallback)
        sh(f"am start -a android.intent.action.VIEW -d '{join_url}' -p '{package}'", root=True)
        time.sleep(1)

    if fallback_url:
        sh(f"am start -a android.intent.action.VIEW -d '{fallback_url}' {package}", root=True)
        time.sleep(1)
        sh(f"am start -a android.intent.action.VIEW -d '{fallback_url}' -p '{package}'", root=True)
